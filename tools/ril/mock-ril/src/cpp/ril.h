...
/*********new struct add by leadcore begin**********/
...
typedef struct {
    ...
    int             gsmCompactAct;
    int             utranAct;
} RIL_PreferredPlmnInfo;
/* FIX "Add preferred plmn list" 2010-12-27 zoufeng end */

/*Leadcore: fix Req00000143, 20110223, chengyuxin, Refresh Operation of USAT*/
typedef struct {
    int ref_mode;
    int length;
    char* pRefresh_ef_list;
}RIL_RefreshParam;

//add by zhushasha 20110521
typedef struct {
    char       sNetwork[8];
    int          type;
} RIL_NetworkList;
...
typedef struct {
    ...
    int             dcs;            //<coding>: integer type, coding type of <name>:0:   GSM 7 bit Default Alphabet 1:   UCS2 mode(original value stored in SIM card£©
    char          *email;       //e-mail address, only effective in USIM card, max length is 64 bytes.
} RIL_IccCardRecord;

/*added for VT feature */
#ifdef VT_ENABLE
typedef enum
{
    PV_VID_TYPE_H263,
...
/*********new struct add by leadcore end**********/
...
/***********RIL_REQUEST add by leadcore begin********************/
/* FIX "Add preferred plmn list" 2010-12-27 zoufeng begin */
#if defined(OPT_LEADCORE_RIL)

#define RIL_REQUEST_LEADCORE_BEGIN 300

/**
 * RIL_REQUEST_SET_PREFERRED_PLMN_LIST
 *
 * Edit the preferred PLMN list in SIM card.
 *
 * "data" is RIL_PreferredPlmnInfo
 *
 * "response" is NULL
 *
 * Valid errors:
 ...
 *
 */
#define RIL_REQUEST_SET_PREFERRED_PLMN_LIST 301

/**
 * RIL_REQUEST_GET_PREFERRED_PLMN_LIST
 *
 * Get the preferred PLMN list in SIM card.
 *
 * "data" is NULL
 *
 * "response" is an array of RIL_PreferredPlmnInfo
 *
 * Valid errors:
 *  SUCCESS
 *  GENERIC_FAILURE
 *
 */
#define RIL_REQUEST_GET_PREFERRED_PLMN_LIST 302

#endif /* #if defined(OPT_LEADCORE_RIL) */
/* FIX "Add preferred plmn list" 2010-12-27 zoufeng end */

//<UH, 2011-02-21, lifangdong, Add ContactsAttribute./
#define RIL_REQUEST_GET_RECORD_NUM 303

/*Leadcore: fix Req00000143, 20110223, chengyuxin, Refresh Operation of USAT: RIL_REQUEST_REFRESH_OPERATION*/
#define RIL_REQUEST_REFRESH_OPERATION 304

/* FIX L1809OG_Bug00000405 2011-03-17 zoufeng begin */
/**
 * RIL_REQUEST_SHUT_DOWN_RADIO
 *
 * Request to shut down the radio power.
 *
 * "data" is NULL
 *
 * "response" is NULL
 ...
 *
 */
#define RIL_REQUEST_SHUT_DOWN_RADIO 305
/* FIX L1809OG_Bug00000405 2011-03-17 zoufeng end */

#define RIL_REQUEST_GET_ICC_PHONEBOOK_RECORD_INFO 306

/*Fixed Bug00000657, by zhushasha, 20110426 begin*/
#define RIL_REQUEST_PLAY_CALLWAIT_TONE       307
/*Fixed Bug00000657, by zhushasha, 20110426 end*/

//<UH, 2011-07-13, lifangdong, Read USIM records with "AT^SCPBR"./
#define RIL_REQUEST_READ_ICC_CARD_RECORD       308
#define RIL_REQUEST_WRITE_ICC_CARD_RECORD     309

//added for black incoming call
#define RIL_REQUEST_COMFIRM_INCOMMING_CALL  310

/**
 * RIL_REQUEST_QUERY_PIN_REMAIN_ATTEMPT
 *
 *Queries pin1/pin2/puk1/puk2
 *
 * "data" is NULL
 *
  * "response" is int *
 * ((int *)response)[0] is pin1
 * ((int *)response)[1] is pin2
 * ((int *)response)[2] is puk1
 * ((int *)response)[3] is puk2
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 ...
 *
 */
#define RIL_REQUEST_QUERY_PIN_REMAIN_ATTEMPT 311

/**
 * RIL_REQUEST_VT_DIAL
 *
 * Dial a video phone out.
 *
 * "data" is int *
 * ((int *)data) is the phone number.
 *
 * "response" is NULL
 *
 * Valid errors:
 ...
 *
 */
#define RIL_REQUEST_VT_DIAL                 312 

/**
 * RIL_REQUEST_VT_HANGUP
 *
 * Hangup the video phone.
 *
 * "data" is NULL 
 *
 * "response" is NULL
 ...
 *
 */
#define RIL_REQUEST_VT_HANGUP               313

/**
 * RIL_REQUEST_VT_ANSWER
 *
 * Answer the incoming video phone call.
 *
 * "data" is NULL 
 *
 * "response" is NULL
 ...
 *
 */
#define RIL_REQUEST_VT_ANSWER               314 

/**
 * RIL_REQUEST_VT_SEND_STRS
 *
 * Send the user input cmds to remote.
 *
 * "data" is const char * containing the userinput cmds. 
 *
 * "response" is NULL
 *
 * Valid errors:
 ...
 *
 */
#define RIL_REQUEST_VT_SEND_STRS            315 

/**
 * RIL_REQUEST_VT_SENDCMD
 *
 * Send the audio/video cmds to remote.
 *
 * "data" is int *
 * ((int *)data)[0] is 1 if send video data to remote 
 *                  is 0 if send audio data to remote
 * ((int *)data)[1] is 1 if audio/video is started 
 *                     0 if audio/video is closed
 * "response" is NULL
 *
 * Valid errors:
 *  SUCCESS
 ...
 *
 */
#define RIL_REQUEST_VT_SENDCMD              316

/** Enh00000171, add checkCOL, by gaofeng 2012-08-30 begin */
#define RIL_REQUEST_SET_COLP 317
#define RIL_REQUEST_GET_COLR 318
#define RIL_REQUEST_GET_COLP 319
/** Enh00000171, add checkCOL, by gaofeng 2012-08-30 end */

/***********RIL_REQUEST add by leadcore end********************/
...
/***********RIL_UNSOL add by leadcore begin********************/

#define RIL_UNSOL_LEADCORE_BEGIN 1036

/*add by mayingzi 2009-04-16 begin*/
/**
 * RIL_UNSOL_RESPONSE_SIM_TYPE
 *
 * SIM TYPE indication,when we send AT^DTOC and get its response, send this
*  UNSOL command
 *
 * "data" is an int*
 */
#define RIL_UNSOL_RESPONSE_SIM_TYPE RIL_UNSOL_LEADCORE_BEGIN
/*add by mayingzi 2009-04-16 end*/
/**
 * RIL_UNSOL_RESPONSE_VT_RING
 *
 * Ring indication for an incoming video call.
 *
 *
 * "data" is null
 */
//#define RIL_UNSOL_RESPONSE_VT_RING      1031

/**
 * RIL_UNSOL_RESPONSE_VT_STRRI
 *
 * Called when userinput cmds is received.
 *
 * "data" is const char *
 *
 */
#define RIL_UNSOL_RESPONSE_VT_STRRI (RIL_UNSOL_LEADCORE_BEGIN + 1)

/**
 * RIL_UNSOL_RESPONSE_VT_SENDI
 *
 * Called when audio/video send cmds is received.
 *
 * "data" is const char *
 *
 */
#define RIL_UNSOL_RESPONSE_VT_SENDI (RIL_UNSOL_LEADCORE_BEGIN + 2)

/**
 * RIL_UNSOL_RESPONSE_VT_RECAI
 *
 * Called when video record info is received.
 *
 * "data" is const char *
 *
 */
#define RIL_UNSOL_RESPONSE_VT_RECAI (RIL_UNSOL_LEADCORE_BEGIN + 3)

/**
 * RIL_UNSOL_RESPONSE_VT_DATA
 *
 * Called when 324 need one I frame.
 *
 * "data" is null
 *
 */
#define RIL_UNSOL_RESPONSE_VT_DATA (RIL_UNSOL_LEADCORE_BEGIN + 4)

/**
 * RIL_UNSOL_RESPONSE_VT_DATA
 *
 * Called when 324 stack is connected.
 *
 * "data" is null
 *
 */
#define RIL_UNSOL_RESPONSE_VT_CODEC (RIL_UNSOL_LEADCORE_BEGIN + 5)

/**
 * RIL_UNSOL_MO_DISCONNECT_CAUSE
 *
 * Called when 324 stack is connected.
 *
 * "data" is response[0] (index)
 *           response[1] (cause)
 *
 */
#define RIL_UNSOL_MO_DISCONNECT_CAUSE (RIL_UNSOL_LEADCORE_BEGIN + 6)

#define RIL_UNSOL_USIM_RECORD_READY (RIL_UNSOL_LEADCORE_BEGIN + 7)

#define RIL_UNSOL_SIM_SMS_STORAGE_AVAILABLE (RIL_UNSOL_LEADCORE_BEGIN + 8)

#define RIL_UNSOL_INCOMING_MT_CALL (RIL_UNSOL_LEADCORE_BEGIN + 9)

/***********RIL_UNSOL add by leadcore end********************/
