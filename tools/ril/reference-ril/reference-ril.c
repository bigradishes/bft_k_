 ...
 */
/* 在这个函数里，每一个RIL_REQUEST_XXX请求都被转化成相应的AT COMMAND,发送给
 * MODEM,然后睡眠等待。当收到此AT COMMAND的最终响应后，线程被唤醒，将响应
 * 传给客户进程(RIL_onRequestComplet->sendResponse)
 *
 */
static void
onRequest (int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    switch (request) {
        ...
        case 115:
            p_response = NULL;
            err = at_send_command_numeric("AT+ZICCID", &p_response);
 ...
 * not be issued here
 */
 /* 这个函数处理MODEM从网络端收到的各种事件，如网络信号变化，拨入的电话，收到短信等。
  * 然后将事件传给客户进程(RIL_onUnsolicitedResponse->sendResponse)
  */
