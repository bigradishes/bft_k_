...
/**********added by Boris 20170401**********/
#define ANDROID5_1_SUPPORTED
#define ANDROID5_1_AND_6_0_SUPPORTED
/*******************************************/
...
typedef enum {
    ...
    PREF_NET_TYPE_LTE_GSM_WCDMA            = 9, /* LTE, GSM/WCDMA */ /*GSM and LTE ax*/
    PREF_NET_TYPE_LTE_CMDA_EVDO_GSM_WCDMA  = 10, /* LTE, CDMA, EvDo, GSM/WCDMA */
    PREF_NET_TYPE_LTE_ONLY                 = 11, /* LTE only */
    //PREF_NET_TYPE_GSM_TD                   = 12, /* GSM and TD  */
    PREF_NET_TYPE_LTE_CDMA= 12,
#if 0
    PREF_NET_TYPE_LTE_TD                   = 13,  /*LTE/TD  */
    PREF_NET_TYPE_GSM_PREF                 = 14,
    PREF_NET_TYPE_TD_PREF                  = 15,
    PREF_NET_TYPE_TD_ONLY                  = 16,
    PREF_NET_TYPE_TDDLTE_PREF              = 17,
    PREF_NET_TYPE_TDDLTE_ONLY              = 18,
    PREF_NET_TYPE_FDDLTE_PREF              = 19,
    PREF_NET_TYPE_FDDLTE_ONLY              = 20
#endif
...
typedef struct cardinfo_
{
  int usedNum;
  int totalNum; 
} cardinfoStruct;
typedef struct //cardrequest_
{
  int min;
  int max; 
} cardrequestStruct;

typedef struct //cardrecord_
{
    int             index;          //the location of record in phone book, which begin in 1.
    char          *num1;        //max length 32 bytes
    int             type1;         //number type, type of address octet in integer format (refer TS 24.008 [8] subclause 10.5.4.7);
    char          *num2;
    int             type2;
    char          *num3;
    int             type3;
    char          *num4;
    int             type4;
    char          *name;        //coatact name
    int             dcs;            //<coding>: integer type, coding type of <name>:0:   GSM 7 bit Default Alphabet 1:   UCS2 mode(original value stored in SIM card?\A8\BA?
    char          *email;
}CardRecordStruct;
...
typedef struct {//zxj add for emode test
    int syncHoSuccCnt;  /* Gsm sync succ cnt*/
    int asyncHoSuccCnt;  /* Gsm async succ cnt*/
    int preHoSuccCnt;  /* Gsm pre succ cnt*/
    int pseudoHoSuccCnt;  /* Gsm pseudo succ cnt*/
    int syncHoFailCnt;  /* Gsm sync Fail cnt*/
    int asyncHoFailCnt;  /* Gsm async Fail cnt*/
    int preHoFailCnt;  /* Gsm pre Fail cnt*/
    int pseudoHoFailCnt;  /* Gsm pseudo Fail cnt*/
    int hardHoSuccCnt;  /* TD hard succ cnt*/
    int batondoHoSuccCnt;  /* TD baton succ cnt*/
    int hardHoFailCnt;  /* TD hard Fail cnt*/
    int batonHoFailCnt;  /* TD baton Fail cnt*/
    int tdHoGsmSuccCnt;  /* TDHoGsm  succ cnt*/
    int tdHoGsmFailCnt;  /* TDHoGsm  Fail cnt*/
    int gsmHoTdSuccCnt;  /* GsmHoTD  succ cnt*/
    int gsmHoTdFailCnt;  /* GsmHoTD    Fail cnt*/
} RIL_NetworkChangeCnt;
...
typedef struct {
    ...
    int             dcs;    /* 3GPP TS 23.038 [25] Cell Broadcast Data Coding Scheme in integer format (default 15) 
                                *   15 7bit
                                *   68 8bit
                                *   72 16bit
                                */
...
/* structures for extended request */
typedef struct {
    char* sres;
    char* kc;
}RIL_SIM_AUTH_Cnf;
...
typedef struct {
    ...
    char* res_auts; //if status is 0, this is res, if status is 1, this is auts, the others are NULL
    char* ck;  // if status is 0
    char* ik;  // if status is 0
    char* kc; // if status is 0 and GSM environment
} RIL_USIM_AUTH_Cnf;
...
//added 5 request imfo below by Boris for android5.1 20161128
...
//added 22 request imfo below by Boris for android6.0 20170311
...
#define RIL_MAX_REQUEST	RIL_REQUEST_VOICE_RADIO_TECH

#define RIL_REQUEST_EXT_BASE 600

#define RIL_REQUEST_SET_PS_MODE (RIL_REQUEST_EXT_BASE + 1)

#define RIL_REQUEST_SET_PS_TRANSFER (RIL_REQUEST_EXT_BASE + 2)

//mod by zouxiaojie 2013119 for TSP 7510 modem and dual standby begin
#define RIL_REQUEST_GET_PINPUK_RETRIES  (RIL_REQUEST_EXT_BASE + 3)
/* about SIM/USIM Authentication */
/** 
* RIL_REQUEST_DO_SIM_AUTH 
* 
* Authenticate for SIM 
* 
* "data" is const char*: <rand> 
* "response" is const char**: 
*       response[0] is <sres> 
*       response[1] is <kc> 
* 
...
*  GENERIC_FAILURE 
*/
#define RIL_REQUEST_DO_SIM_AUTH  (RIL_REQUEST_EXT_BASE + 4)

/** 
* RIL_REQUEST_DO_USIM_AUTH 
* 
* Authenticate for USIM 
* 
* "data" is const char**: 
*        data[0] is <rand> 
*        data[1] is <autn> 
* "response" is int and const char**: 
...
*             response[2] is <ck> 
*             response[3] is <ik> 
*             response[4] is <kc> (if GSM environment) 
*       if status is 1: 
*             response[1] is <auts> 
*             the others are NULL 
*       if status is 2 or 3 
...
*  GENERIC_FAILURE 
*/
#define RIL_REQUEST_DO_USIM_AUTH (RIL_REQUEST_EXT_BASE + 5)
#define RIL_REQUEST_SIM_SMS_CAPABILITY (RIL_REQUEST_EXT_BASE + 6)

//mod by zouxiaojie 2013119 for TSP 7510 modem and dual standby end
//mod by ax for TSP 7510 begin
#define RIL_REQUEST_PLAY_CALLWAIT_TONE_ZTE  (RIL_REQUEST_EXT_BASE + 7)
#define RIL_REQUEST_SET_PREFERRED_PLMN_LIST_ZTE  (RIL_REQUEST_EXT_BASE+8)
#define RIL_REQUEST_GET_PREFERRED_PLMN_LIST_ZTE  (RIL_REQUEST_EXT_BASE+9)
#define RIL_REQUEST_GET_RECORD_NUM (RIL_REQUEST_EXT_BASE + 10)

#define RIL_REQUEST_GET_ICC_PHONEBOOK_RECORD_INFO (RIL_REQUEST_EXT_BASE + 11)

#define RIL_REQUEST_READ_ICC_CARD_RECORD (RIL_REQUEST_EXT_BASE + 12)

#define RIL_REQUEST_WRITE_ICC_CARD_RECORD (RIL_REQUEST_EXT_BASE + 13)

#define RIL_REQUEST_SET_CGATT (RIL_REQUEST_EXT_BASE + 14)
#define RIL_REQUEST_SET_CGSMS (RIL_REQUEST_EXT_BASE + 15)
#define RIL_REQUEST_DEACTIVATE_PDP_ZTE (RIL_REQUEST_EXT_BASE + 16)
#define RIL_REQUEST_SET_3GQOS_ZTE (RIL_REQUEST_EXT_BASE + 17)
#define RIL_REQUEST_QUERY_3GQOS_ZTE (RIL_REQUEST_EXT_BASE + 18)
#define RIL_REQUEST_GET_SN_ZTE (RIL_REQUEST_EXT_BASE + 19)
#define RIL_REQUEST_SET_NET_ZTE  (RIL_REQUEST_EXT_BASE+20)
#define RIL_REQUEST_GET_NET_ZTE  (RIL_REQUEST_EXT_BASE+21)
#define RIL_REQUEST_SET_TDBAND_ZTE  (RIL_REQUEST_EXT_BASE+22)
#define RIL_REQUEST_GET_TDBAND_ZTE  (RIL_REQUEST_EXT_BASE+23)
//mod by zouxiaojie 2013119 for TSP 7510 modem and dual standby end
	 
#define RIL_REQUEST_SET_MODEM_RESET  (RIL_REQUEST_EXT_BASE+24)
#define RIL_REQUEST_SET_IMEI_ZTE  (RIL_REQUEST_EXT_BASE+25)
#define RIL_REQUEST_SET_SN_ZTE   (RIL_REQUEST_EXT_BASE+26)
#define RIL_REQUEST_DEL_IMEI_ZTE  (RIL_REQUEST_EXT_BASE+27)
#define RIL_REQUEST_DEL_SN_ZTE  (RIL_REQUEST_EXT_BASE+28)
#define RIL_REQUEST_DEL_CAL_ZTE  (RIL_REQUEST_EXT_BASE+29)
#define RIL_REQUEST_SET_CAL_ZTE  (RIL_REQUEST_EXT_BASE+30)
#define RIL_REQUEST_GET_CAL_ZTE  (RIL_REQUEST_EXT_BASE+31)
#define RIL_REQUEST_GET_ADJUST_ZTE  (RIL_REQUEST_EXT_BASE+32)
#define RIL_REQUEST_SET_CFUN_ZTE  (RIL_REQUEST_EXT_BASE+33)
#define RIL_REQUEST_DEL_SENSOR_CAL_ZTE  (RIL_REQUEST_EXT_BASE+34)
#define RIL_REQUEST_SET_SENSOR_CAL_ZTE  (RIL_REQUEST_EXT_BASE+35)
#define RIL_REQUEST_QUERY_PROMODE_ZTE  (RIL_REQUEST_EXT_BASE+36)
#define RIL_REQUEST_GET_LCD_FLAG  (RIL_REQUEST_EXT_BASE+37)
#define RIL_REQUEST_SET_LCD_FLAG  (RIL_REQUEST_EXT_BASE+38)
#define RIL_REQUEST_NEW_SMS_INDICATION  (RIL_REQUEST_EXT_BASE+39)
#define RIL_REQUEST_GET_SIM_TYPE  (RIL_REQUEST_EXT_BASE+40)
#define RIL_REQUEST_GET_MSISDN  (RIL_REQUEST_EXT_BASE+41)
#define RIL_REQUEST_ATTACH_GPRS_ZTE  (RIL_REQUEST_EXT_BASE+42)
#define	RIL_REQUEST_DETACH_GPRS_ZTE  (RIL_REQUEST_EXT_BASE+43)
#define RIL_REQUEST_SET_GSM_INFO_PERIODIC_MODE_ZTE  (RIL_REQUEST_EXT_BASE+44)
#define	RIL_REQUEST_TURN_OFF_GSM_INFO_INDICATOR_ZTE  (RIL_REQUEST_EXT_BASE+45)
#define	RIL_REQUEST_PLAY_DTMF_TONE  (RIL_REQUEST_EXT_BASE+46)
#define	RIL_REQUEST_SHUT_DOWN_RADIO  (RIL_REQUEST_EXT_BASE+47)
#define RIL_REQUEST_SYN_PDP_CONTEXT (RIL_REQUEST_EXT_BASE+48)
#define RIL_REQUEST_DEFINE_PDP_CONTEXT (RIL_REQUEST_EXT_BASE+49)
#define RIL_REQUEST_GET_ZPS_STAT (RIL_REQUEST_EXT_BASE+50)
#define RIL_REQUEST_GET_BGLTE_PLMN (RIL_REQUEST_EXT_BASE +51)
#define RIL_REQUEST_SET_ZFLAG (RIL_REQUEST_EXT_BASE+52)
...
#define RIL_UNSOL_SIM_ICCID 1036
#define RIL_UNSOL_SIM_PHONEBOOK_READY 1037
#define RIL_UNSOL_PS_MOVING 1038//add by wangna for psmoving
#define RIL_UNSOL_GSM_SERVING_CELL_INFO_ZTE    1039
#define RIL_UNSOL_NETWORK_REGISTER_FAILED_ON_TDLTE 1040//add by wangna 20140111 for ps register on gsm
#define RIL_UNSOL_RESPONSE_PDP_SETUP_RESULT 1041
#define RIL_UNSOL_SIM_SMS_STORAGE_AVAILABLE 1042//add by zxj
#define RIL_UNSOL_PDP_SYN_CONTEXT 1043
#define RIL_UNSOL_LTE_MISSED 1044
#define RIL_UNSOL_SET_PS_AIRPLANE 1045
#define RIL_UNSOL_SET_CS_AIRPLANE 1046

#ifdef ZTE_RIL_EXTEND
#define RIL_UNSOL_RESPONSE_EXT_BASE 1050
//#define RIL_UNSOL_SIM_ICCID         RIL_UNSOL_RESPONSE_EXT_BASE +1
//#define RIL_UNSOL_PS_MOVING         RIL_UNSOL_RESPONSE_EXT_BASE +1//delete by wangna for psmoving
//#define RIL_UNSOL_GSM_SERVING_CELL_INFO_ZTE    RIL_UNSOL_RESPONSE_EXT_BASE +3
