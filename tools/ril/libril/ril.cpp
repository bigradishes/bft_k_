...
#define SOCKET_NAME_RIL_PS "rild-ps"

#define SOCKET_NAME_RIL_DEBUG "rild-debug"
#define SOCKET_NAME_RIL_DEBUG_PS "rild-debug-ps"
...
/* Android added by GanHuiliang_2012-7-5 *** BEGIN */
#ifdef ZTE_RIL_EXTEND
static pthread_mutex_t s_rilUnsolResponseList = PTHREAD_MUTEX_INITIALIZER;
typedef struct RIL_UNSOL_RESPONSE_t
{
	int unsolResponse;
	size_t len;
	void* data;
	RIL_UNSOL_RESPONSE_t* next;
} RIL_UNSOL_RESPONSE;
static RIL_UNSOL_RESPONSE *ril_unsol_response = NULL;

static void addToUnsolList(RIL_UNSOL_RESPONSE ** response, RIL_UNSOL_RESPONSE ** list)
{
	RIL_UNSOL_RESPONSE *pCur = NULL;

	if(NULL == list || NULL == response)
	{
		ALOGE("%s:Invalid parameters\n", __FUNCTION__);
		return;
	}

	pthread_mutex_lock(&s_rilUnsolResponseList);
	if(NULL == *list)
	{
		*list = *response;
	}
	...
	pthread_mutex_unlock(&s_rilUnsolResponseList);

	return;
}

static void freeUnsolResponse(RIL_UNSOL_RESPONSE ** response)
{
	RIL_UNSOL_RESPONSE *p = NULL;
...
static void freeUnsolResponseList(RIL_UNSOL_RESPONSE ** list)
{
	RIL_UNSOL_RESPONSE *pList = NULL;
	RIL_UNSOL_RESPONSE *pCur = NULL;
	...
	while(NULL != pList)
	{
		pCur = pList;
		pList = pList->next;
		freeUnsolResponse(&pCur);
	}
	pthread_mutex_unlock(&s_rilUnsolResponseList);

	ALOGD("%s: free unsol list done!!!\n", __FUNCTION__);

	return;
}
#endif /* ZTE_RIL_EXTEND */
/* Android added by GanHuiliang_2012-7-5 END */
...
/* dispatch系列函数，此函数接收来自客户进程的命令与相应参数，并调用onRequest
 * 进行处理
 */
...
static void dispatchIccWrite (Parcel& p, RequestInfo *pRI);
...
static void dispatchRilCdmaSmsWriteArgs(Parcel &p, RequestInfo *pRI);
    ...
    //added 22 dispatch  by Boris for android6.0 20170311
#if 1 
...
/* response系列函数，对于每一个命令，都规定了一个对应的response函数来处理
 * 命令的最终响应；对于每一个网络端的事件，也规定了一个对应的response函数来
 * 处理此事件。response函数可被onUnsolicitedResponse或者onRequestComplete调用。
 */
...
#ifdef ZTE_RIL_EXTEND
static int responseReadIccCardRecord(Parcel &p, void *response, size_t responselen);
static int responseNum(Parcel &p, void *response, size_t responselen);
static int responseSimAuth(Parcel &p, void *response, size_t responselen);
static int responseUsimAuth(Parcel &p,void *response, size_t responselen);
static void dispatchPreferredPlmn(Parcel &p, RequestInfo *pRI);
static int responsePreferredPlmnList(Parcel &p,void *response, size_t responselen);
#endif /* ZTE_RIL_EXTEND */
...
processCommandBuffer(void *buffer, size_t buflen) {
    ...
    if (request < 1 || request >= (int32_t)NUM_ELEMS(s_commands)) {
		...
		p.writeInt32(token);
		p.writeInt32(RIL_E_GENERIC_FAILURE);
		sendResponse(p);
		/*----------------------------------*/
		ALOGE("unsupported request code %d token %d", request, token);
#else
    	RIL_onRequestComplete(pRI,RIL_E_GENERIC_FAILURE, NULL, 0);//Open this sentense for android6.0 by Boris.and modify token to pRI, RIL_E_SUCCESS to RIL_E_GENERIC_FAILURE,20170318
		ALOGE("Using RIL_onRequestComplete in zte-ril onRequest for unsupported request code %d token %d, send RIL_E_GENERIC_FAILURE", request, token);// for android6.0 by Boris.add send RIL_E_GENERIC_FAILURE,20170318
#endif		
...
dispatchUssd (Parcel &p, RequestInfo *pRI) {
    ...
    ussd.ussdStr = strdupReadString(p);
    
    startRequest;
    appendPrintBuf("%smode=0x%X,dcs=0x%X,ussdStr=%s", printBuf,
        ussd.mode, ussd.dcs, (char*)ussd.ussdStr);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);
...
static void dispatchIccWrite(Parcel &p, RequestInfo *pRI) {
	...
	CardRecordStruct   args;
    int32_t                 t;
    status_t                status;

    memset(&args, 0x00, sizeof(CardRecordStruct));

    status = p.readInt32(&(args.index));
    args.num1 = strdupReadString(p);
    status = p.readInt32(&(args.type1));
    args.num2 = strdupReadString(p);
    status = p.readInt32(&(args.type2));
    args.num3 = strdupReadString(p);
    status = p.readInt32(&(args.type3));
    args.num4 = strdupReadString(p);
    status = p.readInt32(&(args.type4));
    args.name = strdupReadString(p);
    status = p.readInt32(&(args.dcs));
    args.email = strdupReadString(p);

    startRequest;
    appendPrintBuf("%s[index=%d, num1=%s, type1=%d, num2=%s, type2=%d, num3=%s, type3=%d, num4=%s, type4=%d, num3=%s, dcs=%d, email=%s",
            printBuf,
            args.index,
            args.num1,
            args.type1,
	...
	ALOGD("dispatchIccWrite  %d ",args.index);
    s_callbacks.onRequest(pRI->pCI->requestNumber, &args, sizeof(args), pRI);
...
static void dispatchNetworkSelectionManual(Parcel &p, RequestInfo *pRI) {
    ...
    status = p.readInt32(&t);
    NetworkSel.mode = (int)t;

    NetworkSel.operLongStr = strdupReadString(p);

    NetworkSel.operShortStr = strdupReadString(p);

    NetworkSel.operNumStr = strdupReadString(p);

    status = p.readInt32(&t);
    NetworkSel.act = (int)t;
...
sendResponseRaw (const void *data, size_t dataSize) {
	...
	//ALOGD("send parcel to %s",socketName);

    ret = blockingWrite(fd, (void *)&header, sizeof(header));

    if (ret < 0) {
        //command fd disconnected
        close(s_fdCommand);
        s_fdCommand = -1;
...
static void triggerEvLoop() {
	 ...
	 * int pthread_equal(pthread_t threadid1, pthread_t thread2):
	 * 判断两个线程ID是否相等，返回0 不相等，非零相等
	 */
...
static int responseSimAuth(Parcel &p,void *response, size_t responselen)
{
	...
	if (responselen != sizeof(RIL_SIM_AUTH_Cnf)) {
		ALOGE("invalid response length %d expected %d\n", (int)responselen, (int)sizeof(RIL_USIM_AUTH_Cnf));
		return RIL_ERRNO_INVALID_RESPONSE;
	}
	RIL_SIM_AUTH_Cnf *p_cur = (RIL_SIM_AUTH_Cnf *) response;
	writeStringToParcel(p, p_cur->sres);
	writeStringToParcel(p, p_cur->kc);
	startResponse;
	appendPrintBuf("%s sres:%s,ck:%s", printBuf, (p_cur->sres)?(p_cur->sres):"NULL", (p_cur->kc)?(p_cur->kc):"NULL");
	closeResponse;
	return 0;
}
static int responseNum(Parcel &p,void *response, size_t responselen)
{  
	...
	if (responselen != sizeof(cardinfoStruct)) {
		ALOGE("invalid response length %d expected %d\n", (int)responselen, (int)sizeof(cardinfoStruct));
		return RIL_ERRNO_INVALID_RESPONSE;
	}
	cardinfoStruct *p_cur = (cardinfoStruct *) response;
	p.writeInt32(2);
	p.writeInt32(p_cur->usedNum);
	p.writeInt32(p_cur->totalNum);
	startResponse;
	appendPrintBuf("%s usedNum:%d,totalNum:%d", printBuf, p_cur->usedNum, p_cur->totalNum);
	closeResponse;
	return 0;
}
static int responseReadIccCardRecord(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen % sizeof(CardRecordStruct) != 0) {
        ALOGE("invalid response length %d expected multiple of %d",
                (int)responselen, (int)sizeof(CardRecordStruct));
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    int num = responselen / sizeof(CardRecordStruct);
    
    p.writeInt32(num);

    CardRecordStruct *p_cur = (CardRecordStruct *) response;
    startResponse;
    int i;
    for (i = 0; i < num; i++) {
        p.writeInt32(p_cur[i].index);
        writeStringToParcel(p, p_cur[i].num1);
        p.writeInt32(p_cur[i].type1);
        writeStringToParcel(p, p_cur[i].num2);
        p.writeInt32(p_cur[i].type2);
        writeStringToParcel(p, p_cur[i].num3);
        p.writeInt32(p_cur[i].type3);
        writeStringToParcel(p, p_cur[i].num4);
        p.writeInt32(p_cur[i].type4);
        writeStringToParcel(p, p_cur[i].name);
        p.writeInt32(p_cur[i].dcs);
...
/*static int responseicc(Parcel &p,void *response, size_t responselen)
{
	if (response == NULL && responselen != 0) {
		LOGE("invalid response: NULL");
		return RIL_ERRNO_INVALID_RESPONSE;
	}
	if (responselen != sizeof(CardRecordStruct)) {
		LOGE("invalid response length %d expected %d\n", (int)responselen, (int)sizeof(RIL_USIM_AUTH_Cnf));
		return RIL_ERRNO_INVALID_RESPONSE;
	}
	CardRecordStruct *p_cur = (CardRecordStruct *) response;
	p.writeInt32(p_cur->index);
	writeStringToParcel(p, p_cur->num1);
	p.writeInt32(p_cur->type1);
	writeStringToParcel(p, p_cur->num2);
	p.writeInt32(p_cur->type2);
	writeStringToParcel(p, p_cur->num3);
	p.writeInt32(p_cur->type3);
	writeStringToParcel(p, p_cur->num4);
	p.writeInt32(p_cur->type4);
	writeStringToParcel(p, p_cur->name);
...
static int responseUsimAuth(Parcel &p,void *response, size_t responselen)
{
	...
	if (responselen != sizeof(RIL_USIM_AUTH_Cnf))
	{
		ALOGE("invalid response length %d expected %d\n", (int)responselen, (int)sizeof(RIL_USIM_AUTH_Cnf));
		return RIL_ERRNO_INVALID_RESPONSE;
	}
	RIL_USIM_AUTH_Cnf *p_cur = (RIL_USIM_AUTH_Cnf *) response;
	p.writeInt32(p_cur->status);
	writeStringToParcel(p, p_cur->res_auts);
	writeStringToParcel(p, p_cur->ck);
	writeStringToParcel(p, p_cur->ik);
	writeStringToParcel(p, p_cur->kc);
	startResponse;
	appendPrintBuf("%s%d,%s,%s,%s,%s", printBuf, p_cur->status, 
									(char*)p_cur->res_auts, 
									(p_cur->ck)?(p_cur->ck):"NULL", 
									(p_cur->ik)?(p_cur->ik):"NULL",
									(p_cur->kc)?(p_cur->kc):"NULL");
	closeResponse;
	return 0;
}
...
static void dispatchPreferredPlmn(Parcel &p, RequestInfo *pRI)
{
	...
	plmn.oper = strdupReadString(p);

	status = p.readInt32(&t);
	plmn.GSM_AcT = (int)t;

	status = p.readInt32(&t);
	plmn.GSM_Compact_AcT = (int)t;

	status = p.readInt32(&t);
	plmn.UTRAN_AcT = (int)t;
	...
	startRequest;
	appendPrintBuf("%sindex=%d,format=%d,oper=%s,GSM_AcT=%d,GSM_Compact_AcT=%d,UTRAN_AcT=%d",
					printBuf,
					plmn.index, plmn.format, plmn.oper, plmn.GSM_AcT,plmn.GSM_Compact_AcT,plmn.UTRAN_AcT);
	closeRequest;
	printRequest(pRI->token, pRI->pCI->requestNumber);

	s_callbacks.onRequest(pRI->pCI->requestNumber, &plmn, sizeof(plmn), pRI);

	#ifdef MEMSET_FREED
	memsetString(plmn.oper);
	#endif
	free (plmn.oper);
...
invalid:
    ...
    //added  by Boris for android6.0 20170318
#if 1
/**
...
dispatchSIM_APDU (Parcel &p, RequestInfo *pRI) {
    int32_t t;
    status_t status;
#if 0  //deleted by Boris tmp for DINGWEI no sim bug,20170318
    RIL_SIM_APDU apdu;
...
static int responsePreferredPlmnList(Parcel &p, void *response, size_t responselen)
{
	...
	if (responselen % sizeof (Preferred_PLMN *) != 0)
	{
		ALOGE("invalid response length %d expected multiple of %d\n",
				(int)responselen, (int)sizeof (Preferred_PLMN *));
		return RIL_ERRNO_INVALID_RESPONSE;
	}

	/* number of call info's */
	num = responselen / sizeof(Preferred_PLMN *);
	p.writeInt32(num);
	for (int i = 0 ; i < num ; i++)
	{
		Preferred_PLMN *p_cur = ((Preferred_PLMN **) response)[i];
		/* each plmn info */
		p.writeInt32(p_cur->index);
		p.writeInt32(p_cur->format);
		writeStringToParcel(p, p_cur->oper);
		p.writeInt32(p_cur->GSM_AcT);
		p.writeInt32(p_cur->GSM_Compact_AcT);
		p.writeInt32(p_cur->UTRAN_AcT);
	}
...
/* Android added by GanHuiliang_2012-7-5 *** BEGIN */
#ifdef ZTE_RIL_EXTEND
	freeUnsolResponseList(&ril_unsol_response);
#endif /* ZTE_RIL_EXTEND */
/* Android added by GanHuiliang_2012-7-5 END */
}

/* 具体处理来自客户进程的命令请求。对于每一个命令，ril_commands.h
 * 中都规定了对应的命令处理函数(dispatchXXX)
 */
static void processCommandsCallback(int fd, short flags, void *param) {
	...
	//ALOGD("Rcv parcel from %s",socketName);
...
static void onNewCommandConnect() {
	...
	ALOGI("libril: onNewCommandConnect %s",socketName);
...
/* Android added by GanHuiliang_2012-7-5 *** BEGIN */
#ifdef ZTE_RIL_EXTEND
	RIL_UNSOL_RESPONSE *pTemp = NULL;
	int ret = -1;
	pthread_mutex_lock(&s_rilUnsolResponseList);
	while(NULL != ril_unsol_response)
	{
		//ALOGD("%s: release unsol = %d", __FUNCTION__, ril_unsol_response->unsolResponse);
		ret = sendResponseRaw(ril_unsol_response->data, ril_unsol_response->len);
		if(ret != 0)
		{
			ALOGE("%s: fail unsol = %d", __FUNCTION__, ril_unsol_response->unsolResponse);
		}
		pTemp = ril_unsol_response;
		ril_unsol_response = ril_unsol_response->next;
		freeUnsolResponse(&pTemp);
	}
	pthread_mutex_unlock(&s_rilUnsolResponseList);
#else
...
static void listenCallback (int fd, short flags, void *param) {
	...
	ALOGI("libril: new connection %s",socketName);

//del by angxin 2013119 for TSP 7510 modem and dual standby
    onNewCommandConnect();
    /*if(strcmp(get_process_name(),"rild") == 0)
    {
...
RIL_register (const RIL_RadioFunctions *callbacks) {
 ...
 if(strcmp(get_process_name(),"rild") == 0)
	{
		socketName = SOCKET_NAME_RIL;
	}
	else
	{
		socketName = SOCKET_NAME_RIL_PS;
	}

	s_fdListen = android_get_control_socket(socketName);
	
    if (s_fdListen < 0) {
        ALOGE("Failed to get socket %s",socketName);
        exit(-1);
    }
	...
	{
		s_fdDebug = android_get_control_socket(SOCKET_NAME_RIL_DEBUG_PS);
	}
...
/* 将来自网络端的事件封装(通过responseXXX)后传给客户进程 */
extern "C"
                                ...
                                size_t datalen)
{
	...
	{
		void* pData = NULL;
		RIL_UNSOL_RESPONSE *pRes = NULL;
		pData = (void*)malloc(p.dataSize());
		if(NULL == pData)
		{
			ALOGE("%s: not enough memory for data!!\n", __FUNCTION__);
			return;
		}
		memcpy(pData, p.data(), p.dataSize());
		
		pRes = (RIL_UNSOL_RESPONSE*)malloc(sizeof(RIL_UNSOL_RESPONSE));
		if(NULL == pRes)
		{
			ALOGE("%s: not enough memory for pRes!!\n", __FUNCTION__);
			free(pData);
			pData = NULL;
			return;
		}
		pRes->unsolResponse = unsolResponse;
		pRes->next = NULL;
		pRes->len = p.dataSize();
		pRes->data = pData;
		addToUnsolList(&pRes,&ril_unsol_response);
		ALOGD("%s: save unsol = %d\n", __FUNCTION__, unsolResponse);
	}
#else
...
*/
    switch(request) {
        ...
        //add by zxj 20131121 for TSP 7510 modem and dual standby begin
		...
		/**********GELEI 20170317 and Boris 20170401*******************/
        ...
        case RIL_UNSOL_SIM_ICCID: return "UNSOL_SIM_ICCID";
        case RIL_UNSOL_PS_MOVING: return "UNSOL_PS_MOVING";
		case RIL_UNSOL_GSM_SERVING_CELL_INFO_ZTE: return "UNSOL_GSM_SERVING_CELL_INFO_ZTE";
		case RIL_UNSOL_NETWORK_REGISTER_FAILED_ON_TDLTE: return "RIL_UNSOL_NETWORK_REGISTER_FAILED_ON_TDLTE";
		case RIL_UNSOL_RESPONSE_PDP_SETUP_RESULT: return "RIL_UNSOL_RESPONSE_PDP_SETUP_RESULT";
		case RIL_UNSOL_SIM_SMS_STORAGE_AVAILABLE: return "RIL_UNSOL_SIM_SMS_STORAGE_AVAILABLE";
		case RIL_UNSOL_PDP_SYN_CONTEXT: return "RIL_UNSOL_PDP_SYN_CONTEXT";
		case RIL_UNSOL_LTE_MISSED: return "UNSOL_LTE_MISSED";
		case RIL_UNSOL_SET_PS_AIRPLANE: return "RIL_UNSOL_SET_PS_AIRPLANE";
		case RIL_UNSOL_SET_CS_AIRPLANE: return "RIL_UNSOL_SET_CS_AIRPLANE";


    #ifdef ZTE_RIL_EXTEND
		case RIL_REQUEST_DO_SIM_AUTH: return "SIM_AUTH_ZTE";
		case RIL_REQUEST_DO_USIM_AUTH: return "USIM_AUTH_ZTE";
		//mod by zouxiaojie 2013119 for TSP 7510 modem and dual standby begin
		case RIL_REQUEST_SIM_SMS_CAPABILITY: return "SIM_SMS_CAPABILITY_ZTE";
		case RIL_REQUEST_GET_PINPUK_RETRIES: return "GET_PINPUK_RETRIES_ZTE";
        //mod by zouxiaojie 2013119 for TSP 7510 modem and dual standby end
        //added  by chenguang 20131112 for TSP 7510 modem and dual standby begin
        case RIL_REQUEST_SET_PS_MODE:  return "SET_PS_MODE";
		case RIL_REQUEST_SET_PS_TRANSFER: return "SET_PS_TRANSFER";
		case RIL_REQUEST_PLAY_CALLWAIT_TONE_ZTE: return "PLAY_CALLWAIT_TONE_ZTE";
		case RIL_REQUEST_SET_PREFERRED_PLMN_LIST_ZTE: return "SET_PREFERRED_PLMN_LIST_ZTE";
		case RIL_REQUEST_GET_PREFERRED_PLMN_LIST_ZTE: return "GET_PREFERRED_PLMN_LIST_ZTE";
		case RIL_REQUEST_GET_RECORD_NUM: return "GET_RECORD_NUM";
		case RIL_REQUEST_GET_ICC_PHONEBOOK_RECORD_INFO: return "GET_ICC_PHONEBOOK_RECORD_INFO";
		case RIL_REQUEST_READ_ICC_CARD_RECORD: return "READ_ICC_CARD_RECORD";
		case RIL_REQUEST_WRITE_ICC_CARD_RECORD: return "WRITE_ICC_CARD_RECORD";
		case RIL_REQUEST_SET_CGATT: return "SET_CGATT_ZTE";
		case RIL_REQUEST_SET_CGSMS: return "SET_CGSMS_ZTE";
		case RIL_REQUEST_DEACTIVATE_PDP_ZTE: return "DEACTIVATE_PDP_ZTE";
		case RIL_REQUEST_SET_3GQOS_ZTE: return "SET_3GQOS_ZTE";
		case RIL_REQUEST_QUERY_3GQOS_ZTE: return "QUERY_3GQOS_ZTE";
		case RIL_REQUEST_GET_SN_ZTE: return "GET_SN_ZTE";
		case RIL_REQUEST_SET_NET_ZTE: return "SET_NET_ZTE";
		case RIL_REQUEST_GET_NET_ZTE: return "GET_NET_ZTE";
		case RIL_REQUEST_SET_TDBAND_ZTE: return "SET_TDBAND_ZTE";
		case RIL_REQUEST_GET_TDBAND_ZTE: return "GET_TDBAND_ZTE";
		case RIL_REQUEST_SET_MODEM_RESET: return "SET_MODEM_RESET";
		case RIL_REQUEST_SET_SN_ZTE: return "SET_SN_ZTE";
		case RIL_REQUEST_SET_IMEI_ZTE: return "SET_IMEI_ZTE";
		case RIL_REQUEST_DEL_IMEI_ZTE: return "DEL_IMEI_ZTE";
		case RIL_REQUEST_DEL_SN_ZTE: return "DEL_SN_ZTE";
		case RIL_REQUEST_DEL_CAL_ZTE: return "DEL_CAL_ZTE";
		case RIL_REQUEST_SET_CAL_ZTE: return "SET_CAL_ZTE";
		case RIL_REQUEST_GET_CAL_ZTE: return "SET_IMEI_ZTE";
		case RIL_REQUEST_GET_ADJUST_ZTE: return "GET_ADJUST_ZTE";
		case RIL_REQUEST_SET_CFUN_ZTE: return "SET_CFUN_ZTE";
		case RIL_REQUEST_DEL_SENSOR_CAL_ZTE: return "DEL_SENSOR_CAL_ZTE";
		case RIL_REQUEST_SET_SENSOR_CAL_ZTE: return "SET_SENSOR_CAL_ZTE";
		case RIL_REQUEST_QUERY_PROMODE_ZTE: return "QUERY_PROMODE_ZTE";
		case RIL_REQUEST_GET_LCD_FLAG: return "GET_LCD_FLAG";
		case RIL_REQUEST_SET_LCD_FLAG: return "SET_LCD_FLAG";
		case RIL_REQUEST_NEW_SMS_INDICATION: return "NEW_SMS_INDICATION";
		case RIL_REQUEST_GET_SIM_TYPE: return "GET_SIM_TYPE";
		case RIL_REQUEST_GET_MSISDN:  return "GET_MSISDN";
		case RIL_REQUEST_ATTACH_GPRS_ZTE:  return "ATTACH_GPRS_ZTE";
		case RIL_REQUEST_DETACH_GPRS_ZTE:  return "DETACH_GPRS_ZTE";
		case RIL_REQUEST_SET_GSM_INFO_PERIODIC_MODE_ZTE:  return "SET_GSM_INFO_PERIODIC_MODE_ZTE";
		case RIL_REQUEST_TURN_OFF_GSM_INFO_INDICATOR_ZTE:  return "TURN_OFF_GSM_INFO_INDICATOR_ZTE";
		case RIL_REQUEST_PLAY_DTMF_TONE: return "PLAY_DTMF_TONE";
		case RIL_REQUEST_SHUT_DOWN_RADIO: return "SHUT_DOWN_RADIO";
        //added  by chenguang 20131112 for TSP 7510 modem and dual standby end
        case RIL_REQUEST_SYN_PDP_CONTEXT:return "SYN_PDP_CONTEXT";
		case RIL_REQUEST_DEFINE_PDP_CONTEXT:return "DEFINE_PDP_CONTEXT";
	    case RIL_REQUEST_GET_ZPS_STAT:return "GET_ZPS_STAT";
		case RIL_REQUEST_GET_BGLTE_PLMN: return "GET_BGLTE_PLMN";
		case RIL_REQUEST_SET_ZFLAG:return "SET_ZFLAG";
